...
impl<P: Platform> ServiceResources<P> {
    ...
    pub fn reply_to(&mut self, ctx: &mut CoreContext, request: &Request) -> Result<Reply> {
        ...
        let keystore = move |this: &mut ServiceResources<P>, ctx: &mut CoreContext| {
            drop(assert_once);
            this.keystore(ctx.path.clone())
        };
        let assert_once: [Request; 0] = [];
        let certstore = move |this: &mut ServiceResources<P>, ctx: &mut CoreContext| {
            drop(assert_once);
            this.certstore(ctx)
        };
        let assert_once: [Request; 0] = [];
        let counterstore = move |this: &mut ServiceResources<P>, ctx: &mut CoreContext| {
            drop(assert_once);
            this.counterstore(ctx)
        };
